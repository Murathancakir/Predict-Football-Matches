...
# Set condition
conditions = [(turkish_league.HomeFTGoal + turkish_league.AwayFTGoal > 2),(turkish_league.HomeFTGoal + turkish_league.AwayFTGoal < 2)]
values = [1,0]
turkish_league['HighLow'] = np.select(conditions, values)
...
turkish_league.drop(columns=["Date",HomeGoalsColumn,AwayGoalsColumn,matchResultColumn],inplace=True)
